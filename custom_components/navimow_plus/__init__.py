...
# Remove credentials left behind by the short-lived private map-cloud beta.
# These options are no longer read or sent anywhere.
_LEGACY_PRIVATE_OPTION_KEYS = frozenset(
    {
...
def _remove_legacy_private_options(hass: HomeAssistant, entry: ConfigEntry) -> None:
    """Delete obsolete private mobile-app session material from the entry."""
    cleaned = {
        key: value
        for key, value in entry.options.items()
        if key not in _LEGACY_PRIVATE_OPTION_KEYS
    }
    if len(cleaned) != len(entry.options):
        hass.config_entries.async_update_entry(entry, options=cleaned)
        _LOGGER.info("Removed obsolete private map-cloud session options")
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    _remove_legacy_private_options(hass, entry)
    ...
    try:
        ...
        _LOGGER.info(
            "MQTT connection parameters: broker=%s port=%s credentials=%s",
            mqtt_host,
            mqtt_port,
            "configured" if mqtt_username or mqtt_password or auth_headers else "none",
        )
        ...
        def _attach_mqtt_debug_hooks(sdk: NavimowSDK, api: MowerAPI) -> None:
            ...
            async def _on_connected() -> None:
                _LOGGER.info(
                    "MQTT connected callback: broker=%s port=%s tls=%s client_id=%s",
                    mqtt.broker,
                    mqtt.port,
                    mqtt._use_tls,
                    _get_client_id(),
            ...
            async def _on_disconnected() -> None:
                _LOGGER.debug(
                    "MQTT disconnected callback: broker=%s port=%s tls=%s client_id=%s",
                    mqtt.broker,
                    mqtt.port,
                    mqtt._use_tls,
                    _get_client_id(),
        ...
        async def _async_refresh_mqtt_credentials(
            ...
            except Exception as err:  # noqa: BLE001 - HA OAuth implementations vary
                _LOGGER.warning(
            ...
            if new_auth_headers or new_username or new_password:
                ...
                _LOGGER.info("MQTT credentials refreshed from server")
        ...
        def _create_sdk(api: MowerAPI) -> NavimowSDK:
            ...
            _LOGGER.info(
                "Invoking SDK MQTT connect: broker=%s port=%s transport=%s",
                mqtt_host,
                mqtt_port,
                "websocket" if ws_path else "tcp",
            )
            sdk.connect()
            return sdk
        ...
        _LOGGER.exception("Error setting up Navimow Plus integration")
